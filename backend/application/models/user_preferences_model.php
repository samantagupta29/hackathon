...
class user_preferences_model extends CI_Model {
	...
	public function get_user_preferences($user_id) {
		$this->db->select('c.name, sc.type, u.carbs, u.proteins, u.fats, u.cooking_time, u.cuisine, u.spice_tolerance, u.cooking_style');
		$this->db->from('user_preference u');
		$this->db->join('categories c', 'c.id = u.category_id', 'inner');
		$this->db->join('sub_categories sc', 'sc.id = u.subcategory_id', 'inner');
